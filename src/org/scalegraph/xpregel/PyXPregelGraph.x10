 ...
 * E: Edge value type
 */
public final class PyXPregelGraph[V,E] /*{V haszero,E haszero}*/ {
	private static type XP = org.scalegraph.id.ProfilingID.XPregel;

	val mWorkers :PlaceLocalHandle[PyWorkerPlaceGraph[V,E]];
	val mTeam :Team2;
	...
	public def this(ids :IdStruct)
	{
		...
		mWorkers = PlaceLocalHandle.makeFlat[PyWorkerPlaceGraph[V,E]](mTeam.placeGroup(),
				() => new PyWorkerPlaceGraph[V,E](team, ids));
	}
	...
	public def this(edgeIndexMatrix :DistSparseMatrix[Long])
	{
		...
		mTeam = new Team2(team);
		mWorkers = PlaceLocalHandle.makeFlat[PyWorkerPlaceGraph[V,E]](mTeam.placeGroup(),
				() => new PyWorkerPlaceGraph[V,E](team, edgeIndexMatrix.ids()));
		setGraph(edgeIndexMatrix);
	}
	
	private def this(team :Team, workers :PlaceLocalHandle[PyWorkerPlaceGraph[V,E]])
	{
		mTeam = new Team2(team);
		mWorkers = workers;
	}
	...
	public static def make[V, E](graph :DistSparseMatrix[E]) /*{V haszero, E haszero}*/ {
		val team = Config.get().worldTeam();
		val g = new PyXPregelGraph[V, E](team, PlaceLocalHandle.makeFlat[PyWorkerPlaceGraph[V,E]]
			(team.placeGroup(), () => new PyWorkerPlaceGraph[V, E](team, graph.ids())));
	...
	/** Returns the place group that this PyXPregelGraph is spread on.
	 */
